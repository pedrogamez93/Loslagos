        ...
        <div class="col-md-10">
            <div class="container principal mt-4 mb-4 pt-3 pb-3">
                ...
                <div class="container first-form pt-2 pb-2">
                    ...
                    <form action="{{ route('concursospublicos.update', $concurso->id) }}" method="POST" enctype="multipart/form-data" id="formulario-edicion">
                        @csrf
                        @method('PUT')
                            <!-- Campos del formulario -->
                            <label class="style-label required" for="titulo">Título:</label>
                            <input class="form-control mt-2" type="text" name="titulo" value="{{ $concurso->titulo ?? '' }}">

                            <label class="style-label required" for="tags">Tags:</label>
                            <input class="form-control mt-2" type="text" name="tags" value="{{ $concurso->tags ?? '' }}">
                           
                            <label class="style-label" for="bajada">Bajada o Descripción:</label>
                            <textarea class="form-control mt-2 mb-4" id="" name="descripcion">{{ $concurso->descripcion ?? '' }}</textarea>
...
<script>
    $(document).ready(function() {
        ...
        $("#boton-volver").click(function() {
            window.location.href = "{{ route('concursospublicos.index') }}";
        });
