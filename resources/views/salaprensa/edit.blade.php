        ...
        <div class="col-md-10">
            <div class="container principal mt-4 mb-4 pt-3 pb-3">
                ...
                <div class="container first-form pt-2 pb-2">
                    ...
                    <!-- Formulario para la edición de una noticia -->
                    <form action="{{ route('salaprensa.update', ['id' => $noticias->id]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <input class="form-control mt-2" type="text" name="titulo" placeholder="Título" value="{{ $noticias->titulo }}">
                        ...
                        <select class="form-select mt-4" aria-label="Default select example" name="categoria">
                            ...
                            <option value="Noticia" {{ $noticias->categoria == 'Noticia' ? 'selected' : '' }}>Noticia</option>
                            <option value="Video" {{ $noticias->categoria == 'Video' ? 'selected' : '' }}>Video</option>
                            <option value="Galeria fotografica" {{ $noticias->categoria == 'Galeria fotografica' ? 'selected' : '' }}>Galeria fotografica</option>
                        </select>

                        <div class="mb-3 mt-4">
                            <label for="descripcion" class="form-label">Descripción</label>
                            <textarea class="form-control" name="descripcion" id="descripcion">{{ $noticias->descripcion }}</textarea>
                        </div>

                        <div class="mb-3 mt-4">
                            <label for="formFile" class="form-label">Suba aquí el documento correspondiente</label>
                            <input class="form-control mb-5" type="file" name="archivo_path" id="formFile">
                            @if ($noticias->archivo_path)
                                <p>Archivo actual: <img src="{{ route('mostrar.imagen', ['carpeta' => 'saladeprensa', 'imagen' => basename($noticias->archivo_path)]) }}" ></p>
                            @endif
                        </div>

                        <div class="mb-3 mt-4">
                            <label for="formDate" class="form-label">Fecha:</label>
                            <input class="form-control" type="date" name="fecha" id="formDate" value="{{ $noticias->fecha }}">
                        </div>

                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="destacar" id="flexCheckDefault" {{ $noticias->destacar ? 'checked' : '' }}>
