...
<div class="container-fluid body">
    <div class="row">
        ...
        @if($home->$minibannerField)
            <div class="mb-3 mt-4">
                <label class="style-label mt-4" for="current_image">Imagen Actual:</label>
                <img src="{{ route('mostrar.imagen', ['carpeta' => 'minibanners', 'imagen' => basename($home->$minibannerField)]) }}" class="img-fluid imagen-con-brillo mx-5"  id="currentImage{{ $i }}" style="max-width: 200px; max-height: 200px;">
            </div>
        @endif

        <div class="mb-3 mt-4">
            <label for="formFile" class="form-label">Mini banner {{ $i }}</label>
            <input class="form-control" type="file" name="minibanners{{ $i }}" id="minibanner{{ $i }}" onchange="previewAndReplaceImage(this, 'currentImage{{ $i }}')">
            <input class="form-control mt-2" type="text" name="url_minibanner{{ $i }}" placeholder="{{ $home->$urlMinibannerField ?? '' }}" >
        </div>
    @endfor
