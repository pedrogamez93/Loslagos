        ...
        <div class="col-md-10">
            <div class="container principal mt-4 mb-4 pt-3 pb-3">
                ...
                <div class="container first-form pt-2 pb-2">
                    ...
                    <form id="formulario-creacion" action="{{ route('imagenregion.update', $imagenregion->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="row">
                                <div class="col-md-12 title">
                                    <div class="input-group mb-3">
                                        <input type="text" id="titulo" name="titulo" class="form-control" placeholder="Titulo seccion" value="{{ $imagenregion->titulo }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-12 pb-3">
                                <div id="text">
                                    <div class="form-floating">
                                        <textarea name="bajada" id="editor" class="form-control" placeholder="Escribe tu contenido aquí" id="bajada" style="height: 250px">{!! $imagenregion->bajada !!}</textarea>           
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {{-- Aquí incluyes los documentos existentes para edición o eliminación --}}
                    <h1>Documentos en el registro</h1>
                    @foreach($imagenregion->documentos as $documento)
                        <div class="documento-existente mt-4" style="display: flex; align-items: center; margin-bottom: 10px;">
                            <p style="margin: 0; margin-right: 10px;">.-{{ $documento->nombredoc }}</p>
                            <form action="{{ route('imagenregion-docs.destroy', $documento->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger" sytle="margin-top: 15px;" onclick="return confirm('¿Estás seguro de eliminar este documento?');">Eliminar</button>
                            </form>
                        </div>
                    @endforeach
                </div>
                <a href="{{ route('imagenregion.index') }}" class="btn btn-primary mt-4">Volver al Índice</a>
            </div>
        </div>
    </div>
</div>
